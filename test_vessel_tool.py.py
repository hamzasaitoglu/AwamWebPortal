...
APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbydLi98bvp7q1Lyg4sDnQpzYTt-IFb82vdwYF3qA5ZYSJz-COOdTjm7aWTQP0M0FZD8/exec"
...
with st.sidebar:
    ...
    selected_tool = st.radio("Navigation", [
        ...
        "🚢 Vessel Schedule & Email Generator",
    ...
    <div class='sidebar-footer'>
        ...
        <span style='font-size: 9px; color: #94A3B8;'>v4.0 | agstic.com</span>
    </div>
    """, unsafe_allow_html=True)
            ...
            with st.spinner("Processing RFQ for Awam Operations..."):
                try:
                    ...
                    if "MISSING_CLIENT_NAME" in output_text:
                        st.error("⚠️ Please include the client name in the message body to complete processing!")
...
# ==============================================================================
# MODULE 3: VESSEL SCHEDULE & EMAIL GENERATOR
# ==============================================================================
elif selected_tool == "🚢 Vessel Schedule & Email Generator":
    st.markdown("<div class='awam-header'><div class='awam-title'>🚢 Vessel Schedule & Booking Dispatch Generator</div><div class='awam-subtitle'>Generate official Turkish booking confirmation templates & dispatch via Gmail Client.</div></div>", unsafe_allow_html=True)

    # Top Control Bar: Clear All Inputs Button
    top_bar_col1, top_bar_col2 = st.columns([1, 4])
    with top_bar_col1:
        if st.button("🧹 Clear All Inputs", help="Clear all input fields to start a new draft"):
            st.session_state["v_booking_key"] = ""
            st.session_state["v_pol_key"] = ""
            st.session_state["v_pod_key"] = ""
            st.session_state["v_recipients_key"] = ""
            st.session_state["v_note_key"] = ""
            st.session_state["v_vessel_key"] = ""
            st.session_state["v_cnt_num_key"] = 1
            st.session_state["email_draft_ready"] = False
            st.rerun()

    col1, col2 = st.columns(2, gap="medium")
    
    with col1:
        st.subheader("1. Rezervasyon & Yükleme Detayları")
        v_booking = st.text_input("BOOKİNG NO", key="v_booking_key", value=st.session_state.get("v_booking_key", ""))
        v_pol = st.text_input("YÜKLEME LİMANI (POL)", key="v_pol_key", value=st.session_state.get("v_pol_key", ""))
        v_pod = st.text_input("TAHLİYE LİMANI (POD)", key="v_pod_key", value=st.session_state.get("v_pod_key", ""))
        
        # Split Container Count and Container Type Selectbox (40HC Placed at the top)
        st.markdown("**KONTEYNER ADEDİ / TÜRÜ**")
        cnt_col1, cnt_col2 = st.columns([1, 2])
        with cnt_col1:
            v_cnt_num = st.number_input("Count", min_value=1, step=1, key="v_cnt_num_key", value=st.session_state.get("v_cnt_num_key", 1), label_visibility="collapsed")
        with cnt_col2:
            v_cnt_type_sel = st.selectbox("Type", ["40HC", "20DC", "45HC", "40FR", "40OT", "20FR"], key="v_cnt_type_key", label_visibility="collapsed")
        v_cnt_type = f"{v_cnt_num}X{v_cnt_type_sel}"

        v_recipients = st.text_input("RECIPIENT EMAILS (TO)", key="v_recipients_key", value=st.session_state.get("v_recipients_key", ""), placeholder="e.g. client@example.com, ops@awamlogistics.com", help="Enter recipient emails separated by commas")
        v_note = st.text_input("NOTE / AÇIKLAMA", key="v_note_key", value=st.session_state.get("v_note_key", ""))

    with col2:
        st.subheader("2. Gemi Programı Detayları")
        v_vessel = st.text_input("GEMİ İSMİ / SEFERİ", key="v_vessel_key", value=st.session_state.get("v_vessel_key", ""))
        
        # Date Picker for Ardiyesiz
        d_ardiyesiz = st.date_input("ARDİYESİZ GİRİŞİ", value=None, key="d_ardiyesiz_key")
        v_ardiyesiz = d_ardiyesiz.strftime("%d.%m.%Y") if d_ardiyesiz else "BİLDİRİLECEKTİR"
        
        # Date & Time Pickers for Talimat Cutoff
        st.markdown("**TALİMAT CUTOFF**")
        tc_col1, tc_col2 = st.columns(2)
        with tc_col1:
            d_talimat = st.date_input("Date (Talimat Cutoff)", value=None, key="d_talimat_key", label_visibility="collapsed")
        with tc_col2:
            t_talimat = st.time_input("Time (Talimat Cutoff)", value=datetime.time(12, 0), key="t_talimat_key", label_visibility="collapsed")
        v_talimat_cut = f"{d_talimat.strftime('%d.%m.%Y')}-{t_talimat.strftime('%H.%M')}" if d_talimat else ""

        # Date & Time Pickers for Beyanname Cutoff
        st.markdown("**BEYANNAME CUTOFF**")
        bc_col1, bc_col2 = st.columns(2)
        with bc_col1:
            d_beyanname = st.date_input("Date (Beyanname Cutoff)", value=None, key="d_beyanname_key", label_visibility="collapsed")
        with bc_col2:
            t_beyanname = st.time_input("Time (Beyanname Cutoff)", value=datetime.time(12, 0), key="t_beyanname_key", label_visibility="collapsed")
        v_beyanname_cut = f"{d_beyanname.strftime('%d.%m.%Y')}-{t_beyanname.strftime('%H.%M')}" if d_beyanname else ""

        # Date Picker for ETD / ETA
        d_etd = st.date_input("ETD / ETA", value=None, key="d_etd_key")
        v_etd = d_etd.strftime("%d.%m.%Y") if d_etd else ""

    st.markdown("---")
    
    # Format Subject according to Awam Standards
    email_subject = f"{v_booking} // {v_pol} - {v_pod} // {v_cnt_type}"
    if v_note.strip():
        email_subject += f" // {v_note.strip()}"

    # Auto CC Team Email Setting
    team_cc = "team@awamlogistics.com"

    # Full Rich-Text HTML Template Matching Overview
    html_email_template = f"""
    <div id="email-body-content" style="font-family: Arial, sans-serif; color: #111827; max-width: 700px; line-height: 1.5; font-size: 13.5px; background-color: #FFFFFF; padding: 15px; border-radius: 8px;">
        <p>Merhabalar,</p>
        <p>Rezervasyonunuz oluşturulmuştur.<br>
        Ekipman detayları en kısa süre içerisinde tarafınıza bildirilecektir.<br>
        Aşağıda yer alan detayların kontrolünü rica ederiz;</p>

        <!-- Table 1: Booking Details -->
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
            <tr>
                <td style="width: 35%; background-color: #2A72A4; color: #FFFFFF; font-weight: bold; padding: 8px 12px; border: 1px solid #FFFFFF;">BOOKİNG NO</td>
                <td style="width: 65%; background-color: #F8FAFC; color: #000000; font-weight: bold; padding: 8px 12px; border: 1px solid #E2E8F0;">{v_booking}</td>
            </tr>
            <tr>
                <td style="background-color: #2A72A4; color: #FFFFFF; font-weight: bold; padding: 8px 12px; border: 1px solid #FFFFFF;">YÜKLEME LİMANI (POL)</td>
                <td style="background-color: #F8FAFC; color: #000000; font-weight: bold; padding: 8px 12px; border: 1px solid #E2E8F0;">{v_pol}</td>
            </tr>
            <tr>
                <td style="background-color: #2A72A4; color: #FFFFFF; font-weight: bold; padding: 8px 12px; border: 1px solid #FFFFFF;">TAHLİYE LİMANI (POD)</td>
                <td style="background-color: #F8FAFC; color: #000000; font-weight: bold; padding: 8px 12px; border: 1px solid #E2E8F0;">{v_pod}</td>
            </tr>
            <tr>
                <td style="background-color: #2A72A4; color: #FFFFFF; font-weight: bold; padding: 8px 12px; border: 1px solid #FFFFFF;">KONTEYNER ADEDİ / TÜRÜ</td>
                <td style="background-color: #F8FAFC; color: #000000; font-weight: bold; padding: 8px 12px; border: 1px solid #E2E8F0;">{v_cnt_type}</td>
            </tr>
        </table>

        <p style="font-weight: bold; font-size: 14px; margin-bottom: 8px;">Gemi programı:</p>

        <!-- Table 2: Vessel Schedule -->
        <table style="width: 100%; border-collapse: collapse; text-align: center; margin-bottom: 25px;">
            <thead>
                <tr style="background-color: #2A72A4; color: #FFFFFF; font-size: 11px; font-weight: bold;">
                    <th style="padding: 8px; border: 1px solid #FFFFFF;">GEMİ İSMİ / SEFERİ</th>
                    <th style="padding: 8px; border: 1px solid #FFFFFF;">ARDİYESİZ GİRİŞİ</th>
                    <th style="padding: 8px; border: 1px solid #FFFFFF;">TALİMAT CUTOFF</th>
                    <th style="padding: 8px; border: 1px solid #FFFFFF;">BEYANNAME CUTOFF</th>
                    <th style="padding: 8px; border: 1px solid #FFFFFF;">ETD</th>
                </tr>
            </thead>
            <tbody>
                <tr style="background-color: #F8FAFC; color: #000000; font-weight: bold; font-size: 12px;">
                    <td style="padding: 8px; border: 1px solid #E2E8F0;">{v_vessel}</td>
                    <td style="padding: 8px; border: 1px solid #E2E8F0;">{v_ardiyesiz}</td>
                    <td style="padding: 8px; border: 1px solid #E2E8F0;">{v_talimat_cut}</td>
                    <td style="padding: 8px; border: 1px solid #E2E8F0;">{v_beyanname_cut}</td>
                    <td style="padding: 8px; border: 1px solid #E2E8F0;">{v_etd}</td>
                </tr>
            </tbody>
        </table>

        <!-- Section 3: Important Points -->
        <p style="font-weight: bold; font-size: 14px; margin-bottom: 8px;">Önemli Noktalar;</p>
        <ol style="padding-left: 18px; margin-top: 0; color: #334155; font-size: 11.5px; leading: 1.6;">
            <li>OOG yüklemelerde gemiye yükleme öncesi lashing sertifikası booking referans numaralı mail üzerinden tarafımıza iletilmelidir.</li>
            <li>Tarafınıza bildirmiş olduğumuz konteyner numarasının gelen konteyner üzerindeki numara ile kontrol edilmesini rica ederiz.</li>
            <li>Tarafımıza göndereceğiniz konşimento talimatınız ile beyannamedeki kap adedi/cinsi ve brüt kilo aynı olmalıdır.</li>
            <li>Dolum yapılırken konteynerlerin payloadlarına dikkat edilmesini rica ederiz. Payload aşımlarından doğacak herhangi bir sorundan şirketimiz sorumlu değildir.</li>
            <li>İç taşımasını yapmış olduğumuz yüklemeler için araç bekleme süremiz 6 saattir. Sürenin aşılması durumunda bekleme ücreti tarafınıza yansıtılacaktır.</li>
            <li>Yükleme talimatınızın ve VGM bilgilerinin bildirilen Cut Off sürelerinden önce tarafımıza iletilmesini önemle rica ederiz.</li>
            <li>IMO yüklemelerde kaşe ve imzalı IMO formu booking referans numaralı mail üzerinden tarafımıza iletilmelidir.</li>
            <li>Evrak eksikliği sebebiyle yükün gemiye alınmaması durumunda tarafımızca hiçbir sorumluluk kabul edilmeyecektir.</li>
            <li>İç nakliyenin tarafımızca organize edildiği durumlarda araç bekleme süresi 6 saat olarak belirlenmiştir. Aşıldığı takdirde oluşacak masraf yükleyiciye fatura edilecektir.</li>
            <li>ETA bilgileri geminin operasyon durumuna göre değişiklik gösterebilir.</li>
            <li>Teminatlı yapılacak olan iç taşımalar için araç bekleme süresi konteyner depodan alındığında başlamaktadır.</li>
            <li>Son rezervasyon iptal tarihi sonrasında yapılacak iptallerde USD100 per teu masraf oluşacaktır.</li>
        </ol>
    </div>
    """

    # Direct Gmail URL Link Encoder
    encoded_subject = urllib.parse.quote(email_subject)
    encoded_to = urllib.parse.quote(v_recipients.strip())
    encoded_cc = urllib.parse.quote(team_cc)
    gmail_direct_draft_url = f"https://mail.google.com/mail/?view=cm&fs=1&tf=1&to={encoded_to}&cc={encoded_cc}&su={encoded_subject}"

    # Primary Action Trigger: Process & Direct Copy/Launch
    if st.button("🚀 Generate Email Draft", use_container_width=True, type="primary"):
        if APPS_SCRIPT_URL != "YOUR_GOOGLE_APPS_SCRIPT_WEB_APP_URL_HERE":
            payload = {
                "to": v_recipients.strip(),
                "cc": team_cc,
                "subject": email_subject,
                "htmlBody": html_email_template
            }
            try:
                res = requests.post(APPS_SCRIPT_URL, json=payload)
                if res.status_code == 200:
                    st.success(f"✅ Email draft created successfully in Gmail! (To: {v_recipients.strip() if v_recipients.strip() else 'Unspecified'} | CC: {team_cc})")
                else:
                    st.error("⚠️ Unable to connect to Google Apps Script API. Please check Web App URL configuration.")
            except Exception as e:
                st.error(f"Error: {str(e)}")
        else:
            st.session_state["email_draft_ready"] = True

    if st.session_state.get("email_draft_ready", False):
        st.success(f"✅ Email draft generated successfully! Styled table copied to clipboard. (To: {v_recipients.strip() if v_recipients.strip() else 'Unspecified'} | CC: {team_cc})")
        st.markdown(f'<a href="{gmail_direct_draft_url}" target="_blank" class="gmail-btn">✉️ Click Here to Launch Gmail Draft Window (To: {v_recipients.strip() if v_recipients.strip() else "Direct"} | CC: team@awamlogistics.com)</a>', unsafe_allow_html=True)

    st.subheader("📋 Email Preview & Automatic Copy")
    
    # Auto Copy Component Interop with English Notifications
    copy_script = """
    <script>
    function copyStyledTable() {
        var range = document.createRange();
        range.selectNode(document.getElementById("email-body-content"));
    ...
    <button onclick="copyStyledTable()" style="background-color:#10B981; color:white; border:none; padding:10px 18px; border-radius:6px; font-weight:bold; cursor:pointer; margin-bottom:12px;">📋 Copy Rich HTML Table (With Colors & Formatting)</button>
    """
    full_preview_code = copy_script + html_email_template
    st.components.v1.html(full_preview_code, height=560, scrolling=True)

# ==============================================================================
# MODULE 4: COMPANY DIRECTORY
# ==============================================================================
